...
void display(void) {
	...
	//ModelViewMatrix = glm::rotate(ViewMatrix, 180.0f * TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	{
		float x = (float)i;
		ModelViewMatrix = glm::translate(ViewMatrix, glm::vec3(x, 1.0f/10*x*x, -2));
		ModelViewMatrix = glm::rotate(ModelViewMatrix, atanf(1.0f/5*x), glm::vec3(0.0f, 0.0f, 1.0f));
		ModelViewProjectionMatrix = ProjectionMatrix * ModelViewMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		glLineWidth(2.0f);
		draw_axes();
