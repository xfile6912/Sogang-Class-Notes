...
{
	//TODO:구현하기
	struct bitmap* temp =(struct bitmap*)malloc(sizeof(struct bitmap));
	int i=0;
	size_t prevSize = bitmap_size(bitmap);
	size_t newSize = size + prevSize;
	if (temp != NULL)
	{
		temp->bit_cnt =newSize;
		temp->bits = malloc(byte_cnt(newSize));
		if (temp->bits != NULL || newSize == 0)
		{
			bitmap_set_all(temp, false);//일단 모두 0으로 초기화
			for (i = 0; i < prevSize; i++)
			{
				bitmap_set(temp, i, bitmap_test(bitmap, i));//기존의 bitmap에 있는 원소로 새로 expand한 bitmap 채움.
			}
			free(bitmap);
			return temp;
		}
