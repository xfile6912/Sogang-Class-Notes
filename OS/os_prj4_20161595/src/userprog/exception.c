...
page_fault (struct intr_frame *f) 
{
  ...
  /* Count page faults. */
  page_fault_cnt++;//페이지 폴트 수 증가시킴.

  /* Determine cause. *///어떤 케이스가 page fault 만들었는지 찾음.
  /* Determine cause. */
  bool flag=true;
 ...
 // printf("1\n");
  struct page_entry * page_entry=get_page_entry(&thread_current()->page_table, fault_addr);
  if(page_entry)
  {
	  //      printf("5\n");
	  ...
	  if(frame!=NULL)
	  {
		  swap_in(page_entry->vaddr, frame->paddr, page_entry->swap_idx);
		  page_entry->paddr=frame->paddr;
	  }
	  else
	  {
		  flag=false;
  ...
  else if(fault_addr>=(f->esp -32))
  {
	  ...
	  if(page_num*PGSIZE<=8*1024*1024)//8MB
	  {
		  ...
		  {
			  void *kpage = palloc_get_page(PAL_USER); // frame에서 공간을 새로 할당받음
			  if(!kpage) {
				  evict_frame();
