...
@see _sift_features()
*/
//int sift_features( IplImage* img, struct feature** feat, unsigned char* siftMask )
	...
	int img_dbl, int descr_width, int descr_hist_bins)
{
	...
	/* check arguments */
	if (!img)
		fprintf(stderr, "NULL pointer error, %s, line %d\n", __FILE__, __LINE__);
	...
	octvs = 2;  //////////////////////////////////  #octave overloaded here
...
IplImage* create_init_img(IplImage* img, int img_dbl, double sigma)
{
	...
	if (img_dbl)
	{
		...
		cvSmooth(dbl, dbl, CV_GAUSSIAN, 3, 0, sig_diff, sig_diff);
		cvReleaseImage(&gray);
		return dbl;
	}
	else
	{
		...
		cvSmooth(gray, gray, CV_GAUSSIAN, 3, 0, sig_diff, sig_diff);

		//int yt = 185;
		//int xt = 176;
		//double t_yp1, t_x, t_xp1, t_xm1, t_ym1, t_y;
		//t_x = pixval32f(gray,yt,xt);
		//t_xp1 = pixval32f(gray,yt,xt+1);
		//t_xm1 = pixval32f(gray,yt,xt-1);
		//t_y = pixval32f(gray,yt,xt);
		//t_yp1 = pixval32f(gray,yt+1,xt);
		//t_ym1 = pixval32f(gray,yt-1,xt);
		//printf("\n");
...
IplImage* convert_to_gray32(IplImage* img)
{
	...
	Mat img_mat, gray8_mat;

	int r, c;
	img_mat = cvarrToMat(img);
	...
	if (img_mat.channels() == 1)
		...
		gray8_mat = cvarrToMat(gray8);
		cv::cvtColor(img_mat, gray8_mat, COLOR_BGR2GRAY);
	}
...
double** make_2d_Gaussian_Kernel(int w0, double sigma1) {
	...
	double k_min, k_max;
	double kernel_sum = 0;
	double min = 1.1, max = 0.0;
	double ss;
	double sc;

	ss = 2.0 * sigma1 * sigma1;
	sc = (double)(1.0 / (2 * PI * sigma1 * sigma1));

	///////////////////////////////////////////////////////

	// PROBLEM 1.1 : Implement Gaussian filter.
	...
	for (r = 0; r < w0; r++)
	{
		for (c = 0; c < w0; c++)
		{
			double temp = ((r - w) * (r - w) + (c - w) * (c - w));
			double exponent = -(temp/ ss);
			kernel[r][c] = sc*exp(exponent);
			sum += kernel[r][c];
		}
	}
	for (r = 0; r < w0; r++)
	...
	for (r = 0; r < w0; r++)
		for (c = 0; c < w0; c++)
		{
			if (kernel[r][c] < min) min = kernel[r][c];
			if (kernel[r][c] > max) max = kernel[r][c];
		}
	if (PRB == 1)
		printf("myGaussianKernel[k_size = %d, init sigma = %f] min : %lf, max : %lf\n", w0, sigma1, min, max);

	return kernel;
}

void my_cvSmooth(IplImage* src, IplImage* dst, int w0, double sigma1)
{
	int r, c;
	int m_height = src->height;
	int m_width = src->width;
	int dst_height = dst->height;
	int dst_width = dst->width;
	int src_channels = src->nChannels;					//	1 : grayscale image, 3 : color image(RGB)
	int src_widthstep = src->widthStep;
	int dst_widthstep = dst->widthStep;
	int src_imageSize = src->imageSize;
	int dst_imageSize = dst->imageSize;
	int src_align = src->align;
	int dst_align = dst->align;


	int get_val;
	int w = floor((double)w0 / 2);
	int k, l;
	double** kernel = make_2d_Gaussian_Kernel(w0, sigma1);
	double temp, mean;

	Mat kx = getGaussianKernel(w0, sigma1);
	Mat ky = getGaussianKernel(w0, sigma1);
	...
	for (r = 0; r < m_height; r++) {
		for (c = 0; c < m_width; c++) { 
			...
			// PROBLEM 1.2 : Implement Gaussian filtering to get a blurred image.
			for (int i = -w; i <= w; i++)
			{
				for (int j = -w; j <= w; j++)
				{
					...
					{
						mean += kernel[i + w][j + w] * cvGetReal2D(src, r+i, c+j);//i+w는 0<i+w<w0, j+w는 0<j+w<w0
					}
					else//범위 밖에 있을때
					{
						mean += 0;
	...
	int intvls, double sigma)
{
	...
	gauss_pyr = (IplImage ***)malloc(octvs * sizeof(IplImage**));
	for (i = 0; i < octvs; i++)
	...
	*/
	...
	if (debug) {
		for (i = 0; i < intvls + 3; i++)
			std::printf("sig[%d]=  %f  ", i, sig[i]*255.0);
		std::printf("\n");
	}
	...
	for (o = 0; o < octvs; o++)
		for (i = 0; i < intvls + 3; i++)
		{
			//std::printf("[octave %d][intvls %d]=  %lf\n ", o, i, sig[i]);
			//printf("[%d][%d]:sig[%d]=%f\n", o, i, i, sig[i]);
			if (o == 0 && i == 0)
			...
			else
			{
				...
				cvSmooth(gauss_pyr[o][i - 1], gauss_pyr[o][i], CV_GAUSSIAN, 9, 0, sig[i], sig[i]);
				//my_cvSmooth(gauss_pyr[o][i - 1], gauss_pyr[o][i], 9, sig[i]);
			}
		}
	...
	for (o = 0; o < octvs; o++) {
		for (i = 0; i < intvls + 3; i++) {
			printf("[%d %d, scale]: ", o, i);
			//gauss_pyr[o][i]->width
			for (int r = gauss_pyr[o][i]->height / 2; r < gauss_pyr[o][i]->height / 2 + 1; r++) {
				for (int c = gauss_pyr[o][i]->width / 2; c < gauss_pyr[o][i]->width / 2 + 2; c++) {
					sc = cvGet2D(gauss_pyr[o][i], r, c);
					//if (sc.val[0]>0.01)
					printf(" %f", sc.val[0]);
				}
			}
			...
			for (r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height - SIFT_IMG_BORDER; r++)
				for (c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width - SIFT_IMG_BORDER; c++)
					...
					if (ABS(pixval32f(dog_pyr[o][i], r, c)) > prelim_contr_thr)
						...
						{
							ex1++;
							//printf("extrema %d: (o=%d, s=%d, r=%d, c=%d) ", ex1, o, i, r, c);
							//if (ex1==86)
							//	printf("");
							...
							if (feat)
							{
								...
								//ddata->intvl = intvl;
								//ddata->subintvl = xi;
							//printf("        %d: (o=%d, s=%d, r=%d, c=%d, x=%f, y=%f)\n", ex1, o, ddata->intvl, 
							//	   ddata->r, ddata->c, feat->img_pt.x, feat->img_pt.y);
								if (debug == 1)
									std::printf("    keep (intp), %d\n", nKeep++);
								ex2++;
								...
								if (debug == 1)
									std::printf("    remove (intp), %d\n", nRemove++);
							}
						}

	if (debug == 1) {
		std::printf("\n%d extrema\n", ex1);
		std::printf("%d extrema, interpolate\n", ex2);
		std::printf("%d extrema, non-edge\n", features->total);
	}
	...
	int r, int c, int intvls, double contr_thr)
{
	...
	//printf("extrema : (o=%d, s=%d, r=%d, c=%d)\n", octv, intvl, r, c);
...
CvMat* deriv_3D(IplImage*** dog_pyr, int octv, int intvl, int r, int c)
{
	...
	//printf("\n%f %f\n%f %f\n%f%f\n", pixval32f( dog_pyr[octv][intvl], r, c+1 ), pixval32f( dog_pyr[octv][intvl], r, c-1 ),
	//			pixval32f( dog_pyr[octv][intvl], r+1, c ) , pixval32f(dog_pyr[octv][intvl], r-1, c ),
	//			pixval32f( dog_pyr[octv][intvl+1], r, c ), pixval32f( dog_pyr[octv][intvl-1], r, c ));
	//printf("\n%f \n%f \n%f\n", ( pixval32f( dog_pyr[octv][intvl], r, c+1 ) -
	//		pixval32f( dog_pyr[octv][intvl], r, c-1 ) ),
	//			( pixval32f( dog_pyr[octv][intvl], r+1, c ) -
	//		pixval32f( dog_pyr[octv][intvl], r-1, c ) ),
	//			( pixval32f( dog_pyr[octv][intvl+1], r, c ) -
	//		pixval32f( dog_pyr[octv][intvl-1], r, c ) ));
	//printf("\n%f \n%f \n%f\n", ( pixval32f( dog_pyr[octv][intvl], r, c+1 ) -
	...
	//double t_yp1, t_x, t_xp1, t_xm1, t_ym1, t_y;
	//t_x = pixval32f(dog_pyr[octv][intvl],r,c);
	//t_xp1 = pixval32f(dog_pyr[octv][intvl],r,c+1);
	//t_xm1 = pixval32f(dog_pyr[octv][intvl],r,c-1);
	//t_y = pixval32f(dog_pyr[octv][intvl],r,c);
	//t_yp1 = pixval32f(dog_pyr[octv][intvl],r+1,c);
	//t_ym1 = pixval32f(dog_pyr[octv][intvl],r-1,c);
...
void calc_feature_oris(CvSeq* features, IplImage*** gauss_pyr)
{
	...
	//	feat = CV_GET_SEQ_ELEM( struct feature, features, i );
	//	printf("%d: x=%f, y=%f, ori = %f\n", i, feat->x, feat->y, feat->ori);
	//}
}
	...
	double mag_thr, struct feature* feat)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (hist[i] > hist[l] && hist[i] > hist[r] && hist[i] >= mag_thr)
		{
			...
			//printf("%d: x=%f, y=%f, ori = %f\n", i, new_feat->x, new_feat->y, new_feat->ori);
			...
			*/
			...
			if (rbin > -1.0  &&  rbin < d  &&  cbin > -1.0  &&  cbin < d)
				if (calc_grad_mag_ori(img, r + i, c + j, &grad_mag, &grad_ori))
				{
					...
					//if (debug>=1) {
					//	printf("[d]:  %f %f %f %f\n", c_rot, r_rot, grad_ori, grad_ori); 

					//	for (int p=0; p<4; p++)
					//		for (int q=0; q<4; q++)
					//			for (int w=0; w<8; w++)
					//				printf("%5.8f  ", hist[p][q][w]);
